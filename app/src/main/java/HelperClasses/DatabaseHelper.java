...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        db.execSQL("create table " + TABLE_NAME +" (ID INTEGER PRIMARY KEY AUTOINCREMENT,ACCESS_TOKEN TEXT,USERNAME TEXT,TOKEN_TYPE Text,PASSWORDENC TEXT)");
    }
    ...
    public UserTokens getUserTokenSQL(){
        ...
        Cursor c = db.rawQuery("Select " + COL_1 + "," + COL_2 + "," + COL_3 + "," + COL_4 + "," + COL_5 + " FROM " + TABLE_NAME + " LIMIT 1",null);
        if(c.getCount() > 0){
            if(c.moveToFirst()) {
                ...
                usTok = new UserTokens(AccessToken, Token_type,Username,PassEnc);
                return usTok;
            }
        }
        return null;
    ...
    public boolean updateData(String ACCESS_TOKEN,String USERNAME) {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues contentValues = new ContentValues();
        contentValues.put(COL_2,ACCESS_TOKEN);
